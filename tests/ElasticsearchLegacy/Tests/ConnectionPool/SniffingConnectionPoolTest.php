...
use ElasticsearchLegacy\ConnectionPool\SniffingConnectionPool;
use Mockery as m;
...
class SniffingConnectionPoolTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testAddOneHostAndTriggerSniff()
    {
        ...
        $connections = array($mockConnection);
        $mockNewConnection = m::mock('\ElasticsearchLegacy\Connections\Connection')
                             ->shouldReceive('isAlive')->andReturn(true)->getMock();
    ...
    public function testAddOneHostSniffTwo()
    {
        ...
        $newConnections[] = m::mock('\ElasticsearchLegacy\Connections\Connection')
                             ->shouldReceive('isAlive')->andReturn(true)->getMock();
    ...
    public function testTen_TimeoutNine_SniffTenth_AddTwoAlive()
    {
        ...
        foreach (range(1, 10) as $index) {
            $mockConnection = m::mock('\ElasticsearchLegacy\Connections\Connection')
                              ->shouldReceive('ping')->andReturn(false)->getMock()
                              ->shouldReceive('isAlive')->andReturn(true)->getMock()
                              ->shouldReceive('sniff')->andThrow('ElasticsearchLegacy\Common\Exceptions\Curl\OperationTimeoutException')->getMock();
